...
/**
 * Generates contextual insights for a given calculator result.
 * @param calcId - The calculator identifier
 * @param result - The calculator result object
 * @param inputs - The calculator input values
 * @returns Array of insight objects with icon, title, and description
 */
...
function retirementInsights(result: CalculatorResult, inputs: CalculatorInputs) {
  ...
  r.push({
    ...
    description: "That's ₹" + formatINRFull(monthlySip) + "/month — about " + (dailySaving > 100 ? "a couple of coffees" : "one coffee") + " a day."
  });
  ...
  if (currentAge >= 5) {
    ...
    r.push({
      icon: "⏰",
      title: "Start 5 years earlier, save ₹" + formatINRFull(Math.round(monthlySip * 0.35)) + "/month less",
...
function taxInsights(result: CalculatorResult, inputs: CalculatorInputs) {
  ...
  if (income > 0) {
    ...
    r.push({
      ...
      description: "You pay ₹" + formatINRFull(totalTax) + " in tax on ₹" + formatINRFull(income) + " income."
    });
  }
  const takeHome = income - totalTax;
  r.push({
    icon: "💰",
    title: "You keep ₹" + formatINRFull(takeHome) + " after taxes",
    description: "Out of ₹" + formatINRFull(income) + ", you take home ₹" + formatINRFull(takeHome) + "."
  });
  return r;
}
...
function loanVsInvestInsights(result: CalculatorResult, inputs: CalculatorInputs) {
  ...
  if (diff >= 0) {
    r.push({
      icon: "🏆",
      title: "Investing wins by ₹" + formatINRFull(diff),
      description: "Investing gives you ₹" + formatINRFull(diff) + " more than repaying the loan early."
    });
  } else {
    r.push({
      icon: "✅",
      title: "Repaying wins by ₹" + formatINRFull(Math.abs(diff)),
      description: "Paying off the loan saves you ₹" + formatINRFull(Math.abs(diff)) + " more than investing."
    });
  }
  const totalInterest = result.totalInterest as number;
  r.push({
    icon: "💰",
    title: "Loan costs ₹" + formatINRFull(totalInterest) + " in interest",
    description: "Over " + inputs.years + " years, you pay ₹" + formatINRFull(totalInterest) + " in total interest."
  });
  return r;
}
