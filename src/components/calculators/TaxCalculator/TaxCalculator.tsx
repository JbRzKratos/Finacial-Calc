...
export default function TaxPage() {
  ...
  const handleIncomeInput = useCallback((v: string) => {
    updateInput("income", parseFloat(v.replace(/[,\s]/g, "")) || 0);
  }, [updateInput]);

  const handle80CInput = useCallback((v: string) => {
    updateInput("deduction80C", parseFloat(v.replace(/[,\s]/g, "")) || 0);
  }, [updateInput]);

  const handle80DInput = useCallback((v: string) => {
    updateInput("deduction80D", parseFloat(v.replace(/[,\s]/g, "")) || 0);
  }, [updateInput]);

  const handleHRAInput = useCallback((v: string) => {
    updateInput("hra", parseFloat(v.replace(/[,\s]/g, "")) || 0);
  }, [updateInput]);

  const handleIncomeSlider = useCallback((v: number) => {
    updateInput("income", v);
  }, [updateInput]);
    ...
    <SplitShell
      left={
        <>
          ...
          <InputRow
            fields={[
              { value: Number(inputs.income), onChange: handleIncomeInput, label: "ANNUAL INCOME", step: 50000 },
              { value: Number(inputs.deduction80C), onChange: handle80CInput, label: "80C (PF/ELSS)", step: 10000 },
            ]}
            className="mb-5"
          />
          <InputRow
            fields={[
              { value: Number(inputs.deduction80D), onChange: handle80DInput, label: "80D (MEDICAL)", step: 5000 },
              { value: Number(inputs.hra), onChange: handleHRAInput, label: "HRA", step: 5000 },
            ]}
            className="mb-5"
          />
