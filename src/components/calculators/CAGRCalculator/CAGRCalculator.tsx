...
export default function CAGRPage() {
  ...
  const handleInitialInput = useCallback((v: string) => {
    updateInput("initial", parseFloat(v.replace(/[,\s]/g, "")) || 0);
  }, [updateInput]);

  const handleFinalInput = useCallback((v: string) => {
    updateInput("final", parseFloat(v.replace(/[,\s]/g, "")) || 0);
  }, [updateInput]);
  ...
  const handleInitialSlider = useCallback((v: number) => {
    updateInput("initial", v);
  }, [updateInput]);

  const handleFinalSlider = useCallback((v: number) => {
    updateInput("final", v);
  }, [updateInput]);
    ...
    <SplitShell
      left={
        <>
          ...
          <InputRow
            fields={[
              { value: Number(inputs.initial), onChange: handleInitialInput, label: "INITIAL", step: 1000 },
              { value: Number(inputs.final), onChange: handleFinalInput, label: "FINAL", step: 1000 },
              { value: Number(inputs.years), onChange: handleYearsInput, label: "YEARS", suffix: "yr", step: 1 },
            ]}
            className="mb-5"
          />
          ...
          <NeonSlider
            ...
            onChange={handleInitialSlider}
            min={1000}
            max={10000000}
            step={1000}
            unit="₹"
            editable
            tickLabels={["1K", "25L", "50L", "75L", "1C"]}
          />

          <NeonSlider
            label="FINAL VALUE"
            value={Number(inputs.final)}
            onChange={handleFinalSlider}
            min={1000}
            max={50000000}
            step={1000}
            unit="₹"
            editable
            tickLabels={["1K", "1.25C", "2.5C", "3.75C", "5C"]}
          />
