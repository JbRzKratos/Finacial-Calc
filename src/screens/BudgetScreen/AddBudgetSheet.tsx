...
import { getIcon, ICON_NAMES } from "@/screens/BudgetScreen/iconMap";
...
export function AddBudgetSheet({ open, onClose, onSave }: AddBudgetSheetProps) {
  ...
  useEffect(() => { if (open) { setName(""); setMonthlyLimit(""); setIcon("fork-knife"); setColor("#FF6B00"); } }, [open]);

  const handleSubmit = useCallback(() => {
    if (!name.trim() || !monthlyLimit) return;
    onSave({ name: name.trim(), icon, iconBg: color, monthlyLimit: Number(monthlyLimit), color });
    onClose();
  }, [name, monthlyLimit, icon, color, onSave, onClose]);
