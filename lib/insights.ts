import { InsightData, CalculatorInputs, CalculatorResult } from "@/types/calculator";
import { ruleOf72 } from "./math";

export function generateInsights(calcId: string, result: CalculatorResult, inputs: CalculatorInputs): InsightData[] {
  switch (calcId) {
...
function sipInsights(result: CalculatorResult, inputs: CalculatorInputs): InsightData[] {
  const r: InsightData[] = [];
...
function emiInsights(result: CalculatorResult, inputs: CalculatorInputs): InsightData[] {
  const r: InsightData[] = [];
...
function fdInsights(result: CalculatorResult, inputs: CalculatorInputs): InsightData[] {
  const r: InsightData[] = [];
  const maturity = result.maturity as number;
  const principal = inputs.amount as number;
  if (principal > 0) {
...
function cagrInsights(result: CalculatorResult, inputs: CalculatorInputs): InsightData[] {
  const r: InsightData[] = [];
...
function retirementInsights(result: CalculatorResult, inputs: CalculatorInputs): InsightData[] {
  const r: InsightData[] = [];
  ...
  r.push({
    ...
    description: `We planned your corpus to last from age ${inputs.retirementAge as number} to ${lifeExpectancy}.`
  });
...
function taxInsights(result: CalculatorResult, inputs: CalculatorInputs): InsightData[] {
  const r: InsightData[] = [];
...
function loanVsInvestInsights(result: CalculatorResult, inputs: CalculatorInputs): InsightData[] {
  const r: InsightData[] = [];
