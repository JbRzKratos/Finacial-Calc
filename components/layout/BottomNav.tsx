          ...
          {tabs.map((tab) => {
            ...
            return (
              <Link
                ...
                href={tab.href}
                className={cn(
                  "flex flex-col items-center justify-center gap-0.5 snap-center shrink-0 no-underline transition-all duration-300 rounded-xl min-w-[64px] px-2.5 py-2",
                  active
                    ? "bg-brand-primary text-white shadow-[0_4px_16px_rgba(255,107,0,0.45)] scale-105"
                    : "text-white/50 dark:text-white/40 hover:text-white/80 hover:bg-white/10"
                )}
              >
                <Icon
                  size={18}
